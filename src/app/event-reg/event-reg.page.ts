...
export class EventRegPage implements OnInit {
  ...
  constructor(private formBuilder: FormBuilder, private navController: NavController,private alertController: AlertController,private router: Router,private http: HttpClient)  {
  ...
  sendData(title:string,date:string,dur:number,desc:string,tof1:string,venu1:string ) {
    ...
    console.log(g);
    this.http.post('https://technostoregg.000webhostapp.com/event_list.php/',JSON.stringify(g)).subscribe(
      (response )=> {
        this.showAlert1();
          },
          (error)=>{
